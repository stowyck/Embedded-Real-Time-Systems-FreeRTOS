...
static const uint8_t SevenSegmentDecoderMode[] = {0x78, 0xA8}; //t and d for tune and drive?
...
volatile int8_t dx = 0, dy = 0, cx = 0, cy = 0;

volatile int8_t x = 0;
volatile int8_t y = 0;
volatile uint8_t drive = 0;
volatile int Driving  = 0; //need a motion variable

volatile uint8_t decx = 0;
volatile uint8_t incx = 0;
volatile uint8_t decy = 0;
volatile uint8_t incy = 0;

/*volatile uint8_t LWheelCount = 0;
volatile uint8_t RWheelCount = 0;
volatile uint8_t LStore = 0;
volatile uint8_t RStore = 0;*/
...
static void LEDTask(void *pvParameters){
	for(;;){
		if(xSemaphoreTake(sem_task, portMAX_DELAY)){
			...
			if(xSemaphoreTake(spi_keep, portMAX_DELAY)){// possible problem here with continual access to mutex through the loop, vTaskDelayUntil is crucial
					board7SEG_ASSERT_CS();
						FreeRTOS_write(SPIPort, &(SevenSegmentDecoderMode[MODE]), sizeof(uint8_t));
					board7SEG_DEASSERT_CS();
					xSemaphoreGive(spi_keep);
				}
...
static void TuneTask(void *pvParameters)
{
	...
	{		//becomes play in ISR, but currently not playing then play song
		...
		else if(STATE == PAUSE && SongStarted){
 ...
 *
 *****************************************************************************/
static void WEEEInputTask(void *pvParameters)		//apparently not using?
{
	const portTickType TaskPeriodms =50UL / portTICK_RATE_MS;
	(void)pvParameters;
	uint8_t DriveStarted = 1;
	int8_t deltaX = 0;
	int8_t deltaY = 0;
	int leftTurn = 0;
	int rightTurn = 0;
	//int Driving  = 0; //need a motion variable
	int RealignFromleft = 0;
	int RealignFromright = 0;
	int complete = 0;
	...
	for(;;)
	{
		//set driving to zero when centre joystick pressed call the function in blah blah blah
		//etc, or when we have a match value
	if(drive){

		deltaX = cx - dx;
		deltaY = cy - dy;
		complete = 0;//as soon as we are told to start the task complete has value zero

		if(deltaX == 0 && deltaY == 0 && !complete){

			if(RealignFromright){
				RealignFromright = 0;
				DFR_SetRightWheelDestination(4);
				DFR_SetLeftWheelDestination(4);
				DFR_ClearWheelCounts();
				Driving = 1; decx = 0; incx = 0; incy = 0; decy = 0;
				complete = 1;
				DFR_DriveLeft(100);	//turn robot left again
			}
			else if(RealignFromleft){
				RealignFromleft = 0;
				DFR_SetRightWheelDestination(6);	//turn at right angles may need to change
				DFR_SetLeftWheelDestination(6);
				DFR_ClearWheelCounts();
				Driving = 1; decx = 0; incx = 0; incy = 0; decy = 0;
				complete = 1;
				DFR_DriveRight(100);
			}
			drive = 0; //DFR_ClearWheelCounts();

		}
		else if(complete){//if we have reached our destination
					 drive = 0; decx = 0; incx = 0; incy = 0; decy = 0; Driving = 0; DFR_ClearWheelCounts();
		}
		else if(deltaY > 0 && Driving == 0 && !RealignFromright && !RealignFromleft && !rightTurn && !leftTurn){
			//drive backward
			DFR_SetRightWheelDestination(deltaY);
			DFR_SetLeftWheelDestination(deltaY);
			DFR_ClearWheelCounts();
			Driving = 1; decy = 1; incy = 0; incx = 0; decx = 0;
			DFR_DriveBackward(100);
		}
		else if(deltaY < 0 && Driving == 0 && !RealignFromright && !RealignFromleft && !rightTurn && !leftTurn){
			//drive forwards
			DFR_SetRightWheelDestination(abs(deltaY));
			DFR_SetLeftWheelDestination(abs(deltaY));
			DFR_ClearWheelCounts();
			Driving = 1; incy = 1; decy = 0; incx = 0; decx = 0;
			DFR_DriveForward(100);
		}
		else if(deltaX > 0 && Driving == 0 && !rightTurn && !leftTurn){//turn left //set a pin saying that we drove right on
			DFR_SetRightWheelDestination(6);	//turn at right angles may need to change
			DFR_SetLeftWheelDestination(6);
			//store value of wheel counts
			/*LStore = LWheelCount;
			RStore = RWheelCount;*/
			DFR_ClearWheelCounts();
			Driving = 1; decx = 0; incx = 0; incy = 0; decy = 0;
			leftTurn = 1;											//************************************indicates we just turned left
			DFR_DriveLeft(100);
		}
		else if(leftTurn && Driving == 0 && !rightTurn && !RealignFromright && !RealignFromleft){//if we have just turned left
			leftTurn = 0;
			RealignFromleft = 1;
			/*LWheelCount = LStore; //restoring the count values as to not include the turn
			RWheelCount = RStore;*/
			DFR_SetRightWheelDestination(deltaX);	//then drive forward
			DFR_SetLeftWheelDestination(deltaX);
			DFR_ClearWheelCounts();
			Driving = 1;  decx = 1; incx = 0; decy = 0; incy = 0;
			DFR_DriveForward(100);

		}
		/*else if(RealignFromleft && Driving == 0 && !rightTurn && !leftTurn && !RealignFromright){
			RealignFromleft = 0;
			DFR_SetRightWheelDestination(5);	//turn at right angles may need to change
			DFR_SetLeftWheelDestination(5);
			DFR_ClearWheelCounts();
			Driving = 1; decx = 0; incx = 0; incy = 0; decy = 0;
			complete = 1;
			DFR_DriveRight(100); //turn robot right again
		}*/
		else if(deltaX < 0  && Driving == 0 && !rightTurn && !leftTurn && !RealignFromright && !RealignFromleft){/*Where we are testing...................*/
			DFR_SetRightWheelDestination(6);	//turn at right angles may need to change
			DFR_SetLeftWheelDestination(6);
			DFR_ClearWheelCounts();
			/*LStore = LWheelCount; //store value of wheel counts
			RStore = RWheelCount;*/
			rightTurn = 1;						//indicates we just turned right
			Driving = 1; decx = 0; incx = 0; incy = 0; decy = 0;
			DFR_DriveRight(100);
		}
		else if(rightTurn && Driving == 0 && !leftTurn && !RealignFromright && !RealignFromleft){//if we have just turned right
			rightTurn = 0;
			/*LWheelCount = LStore; //reseting the count values as to not include the turn
			RWheelCount = RStore;*/
			DFR_SetRightWheelDestination(abs(deltaX));	//then drive forward
			DFR_SetLeftWheelDestination(abs(deltaX));
			DFR_ClearWheelCounts();
			RealignFromright = 1;
			Driving = 1; decx = 0; incx = 1; incy = 0; decy = 0;
			DFR_DriveForward(100);
		}
		/*else if(RealignFromright && Driving == 0 && !rightTurn && !leftTurn && !RealignFromleft){
			RealignFromright = 0;
			DFR_SetRightWheelDestination(5);	//turn at right angles may need to change
			DFR_SetLeftWheelDestination(5);
			DFR_ClearWheelCounts();
		...
		//if(DFR_GetLeftWheelCount() == DFR_GetLeftWheelDestination() || DFR_GetLeftWheelCount() == DFR_GetLeftWheelDestination() || drive == 0){/*wheel counts matches wheel destination or centre button is pressed then*/
			/*DFR_DriveStop();
			decx = 0; incx = 0; incy = 0; decy = 0;
			Driving = 0;
			DFR_ClearWheelCounts();
...
static void WEEEDisplayTask(void *pvParameters)
{
	...
	for(;;)
	{
		 sprintf(buffer,"Des: %d,%d ", dx, dy);//destination position
...
int main(void)
{
    ...
    xTaskCreate(WEEEOutputTask,		(const int8_t* const)"Output",		configMINIMAL_STACK_SIZE*5, NULL, 5U, NULL);
...
void EINT3_IRQHandler (void)
{
	...
	if ((((LPC_GPIOINT->IO2IntStatR) >> 11)& 0x1) == ENABLE){
		...
		if (DFR_GetLeftWheelCount() == DFR_GetLeftWheelDestination()) {
			DFR_DriveStop(); Driving = 0; DFR_ClearWheelCounts();
		}
	...
	switch(MODE){
		...
		case DRIVE:
			...
			{
			    //start driving
			    drive = !drive;//use a semaphore to start the task maybe
			    if(drive == 0){DFR_DriveStop(); Driving = 0;}
